...
//! One file drives a Windows-native run and a Wine run: the platform decides how the executable is launched, not the config. Secrets never live in the file; they come from the environment and are held in [`Secret`], which cannot print itself.
...
pub struct Config {
    ...
    /// Kept deserializable indefinitely: seven shipped profiles, `deploy/cellar.toml` and the Kubernetes ConfigMap all use it, and a config that stops parsing is a server that stops starting. Read it through [`Config::instances`] rather than directly, or the `[instances]` spelling is invisible to whatever is reading.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub server: Option<ServerConfig>,

    /// A map keyed by id, not a list: the key is the name, so it cannot be forgotten or duplicated. `BTreeMap` so every listing is stably ordered.
    ...
    /// Process-wide rather than per-instance by default because the case this exists for is a development and a published instance of the *same* gamemode: repeating the profile in both would be two places to forget.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub profile: Option<GamemodeProfile>,

    /// Read `[profile]` out of this file instead, relative to this config.
    /// A profile file keeps gamemode facts beside the config that uses them. It also lets a gamemode ship its own profile next to its `.sbproj`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Restricted to `[a-z0-9][a-z0-9-]{0,31}`. The same string ends up in a URL query value, a Prometheus label, a tracing span and a `VARCHAR(64)` column, and restricting the charset once at parse time is what makes it safe in all four, the same argument `is_valid_identifier` makes for the SQL bootstrap.
...
/// The server settings are nested under `[instances.<id>.server]` rather than flattened. Flattening would read more like `[server]` and cost `deny_unknown_fields`, which serde cannot apply through a flatten, and a silently ignored `hostnam` in the most important table in the file is a worse trade than three extra characters.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct InstanceConfig {
    /// Whose data this is. Defaults to the instance id.
    /// Separate from the id because the id is the routing key and the scope is the storage key. Collapsing them would move an existing deployment's documents the moment it named its instance.
    #[serde(default)]
    pub scope: Option<String>,

    /// Declared but not started. How a Windows-only development instance stays in one config file that also deploys to Linux.
    #[serde(default = "yes")]
    pub enabled: bool,

    /// Whether `/readyz` speaks for this instance.
    /// The highest-consequence flag here: a development instance that cannot start on a host with no editor must not fail readiness for a healthy production server.
    #[serde(default = "yes")]
    pub required: bool,
    ...
    /// Overrides the process-wide `[profile]`, for the rare config whose two instances run different gamemodes.
    #[serde(default)]
    pub profile: Option<GamemodeProfile>,
}
...
/// One instance with every default resolved, which is what the supervisor registry is built from. Produced by [`Config::instances`]; never parsed.
#[derive(Debug, Clone)]
pub struct Instance {
    pub id: InstanceId,
    pub scope: String,
    ...
    /// `None` here means nobody has looked yet, not that there is no ceiling. It is not a config key: `+maxplayers` is not a convar and not a launch switch, the number lives in the project's `Metadata.MaxPlayers`, and letting an operator type a different one here would put a figure on the dashboard that the engine does not enforce. `cellar run` fills it in from the `.sbproj` before building the supervisor.
    pub player_ceiling: Option<u32>,
}

impl Instance {
    /// The line that means this instance is serving.
    /// `server.ready_pattern` wins because it is the narrower statement, then the profile, then the empty default. An empty value means Cellar cannot claim readiness until the operator supplies a line the gamemode logs.
...
/// The default is [`UpdatePolicy::Notify`] rather than `Apply`: taking an update is a decision an operator opts into, not one they discover after the fact.
...
pub struct UpdateConfig {
    ...
    /// Minutes between checks. Frequent enough for a game server, infrequent enough not to hammer a git remote.
    pub check_interval_minutes: u64,
    /// Ask the git remote for its tip. Off keeps every probe local and offline.
    pub check_remote: bool,
    /// Refuse to apply while anybody is connected. The single most important setting here: an updater that restarts a server with people on it is worse than no updater.
    pub only_when_empty: bool,
    /// Local-time hours between which an update may be applied. Equal values mean any time; a wrapping pair (22 to 4) is supported and is when a roleplay server is actually empty.
...
pub struct ServerConfig {
    /// Path to `sbox-server.exe`. A Windows path on Windows, a path inside the Wine prefix's view on Linux.
    pub executable: PathBuf,

    /// The local `.sbproj` handed to `+game` when `game` is unset.
    /// `+game`, not `+project`: `+project` loads metadata and idles at the bare console without ever booting a map.
    #[serde(default)]
    pub project: PathBuf,
    ...
    /// Launcher. Native is the default on Linux and Windows; Wine is an explicit compatibility fallback for Windows binaries.
    #[serde(default)]
    pub launcher: Launcher,

    /// The Wine prefix this instance runs in, passed as `WINEPREFIX`.
    /// Concurrent instances on Linux need one each. Every Wine process in a prefix shares a single `wineserver` which jointly holds their sockets, and `wineserver -k` is prefix-scoped, so one prefix means one instance cannot be dealt with without reaching the other. Unset inherits whatever `WINEPREFIX` Cellar itself was started with, which is right for a single-server host and wrong the moment there are two.
    /// A prefix is not free to create: the server needs the Windows .NET 10 runtime installed into it. `docs/INSTALLATION.md` has the one command.
    #[serde(default)]
    pub wine_prefix: Option<PathBuf>,

    /// Working directory for the child.
    /// It does **not** decide where the engine writes. Measured 2026-09-01: `logs/` and `data/` both follow the executable's own directory, and a working directory elsewhere moved neither. See `docs/ARCHITECTURE.md`.
    #[serde(default)]
    pub working_dir: Option<PathBuf>,

    /// Where Cellar reads `sbox-server.log` from, when it is somewhere other than beside the executable.
    /// A read path, not a write path. Nothing Cellar passes moves the engine's write path, `FACEPUNCH_ENGINE` included, so pointing this at a file the engine does not write means readiness silently never fires.
    #[serde(default)]
    ...
    /// Expose the server's real address instead of routing through Steam's relay. Off by default, matching the existing deployment.
    #[serde(default)]
    ...
    /// Log line that means "serving", overriding whatever the gamemode profile says. Unset falls back to `[profile]` and then to `grammar::DEFAULT_READY_PATTERN`; read it through
    /// [`Instance::ready_pattern`] rather than here.
    /// An `Option` rather than a defaulted `String` so that `ready_pattern = ""` stays meaningful: it is how an operator says this server has no readiness signal at all, and a defaulted field cannot tell that from silence.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Written out rather than derived, because several fields carry a serde `default = "..."` and a derived `Default` would disagree with the file format about `hostname`, `port` and `ready_pattern`.
...
impl ServerConfig {
    /// True when this profile runs a published package rather than a local `.sbproj`.
    ...
    /// The directory the game reads its own state from: `features.json`, `permissions.json` and the `hosting.json` Cellar writes.
    /// `data_dir` is the operator's answer and wins, because it is already what `hosting.json` is written to and pointing the two at different places cannot be right. The fallback reproduces the engine's own layout, `<exe dir>/data/<ident split on dots>`, which is derivable only for a published package: a local `.sbproj` run leaves `game` unset.
    ...
    /// `Logging.cs` builds `{base}/logs/{processName}.log`, and `{base}` is `AppContext.BaseDirectory`: the executable's own directory. It reads `FACEPUNCH_ENGINE` first, but with `EnvironmentVariableTarget.User`, which is the Windows registry rather than the process environment, and setting it in either was measured to move nothing. `server.log_file` overrides Cellar's read path and does not move the engine's write path.
    ...
    /// The engine appends `#local` to the ident for a local `.sbproj` and names the data directory after the ident, so a profile carrying the other mode's leaf still starts: `hosting.json` goes where nothing reads it, and neither mode can see the other's characters, permissions or features. Every AppleJack Framework profile shipped with the `#local` leaf, published ones included, until 2026-08-28.
...
pub struct SupervisorConfig {
    ...
    /// The engine installs no SIGTERM handler, so `quit` typed at the console is the only clean stop there is, and it has nine shutdown steps to get through including a Steam logoff. Kubernetes must be given at least this much `terminationGracePeriodSeconds`.
    pub graceful_timeout_seconds: u64,

    /// How often to sample process CPU and memory.
    pub sample_interval_seconds: u64,

    /// How long a server may sit in `Starting` before Cellar says it is not working. `0` disables the check.
    /// A server that never becomes ready is otherwise indistinguishable from one still starting, and both observed causes are permanent: a `ready_pattern` the gamemode never emits, and an engine that fails to resolve a package and then idles at the console at 165% CPU instead of exiting. The deadline expiring never kills or restarts the child, because a wrong pattern in front of a healthy server is one of the two cases and killing it would be the wrong answer to it.
...
impl Default for SupervisorConfig {
    fn default() -> Self {
        Self {
            ...
            // A cold facepunch.sandbox start measured about 120 seconds on the Arch box, and a first run also compiles packages. Ten minutes is far enough above that to be a statement about a permanent condition rather than a slow disk.
...
pub struct BridgeConfig {
    ...
    /// Address to bind. Loopback by default, which is what makes `trusted` auth defensible.
    pub bind: String,

    /// The URL written into `hosting.json` for the gamemode to call.
    /// Must be the final URL. The engine strips `Authorization` on every redirect hop, so a bridge URL that redirects loses its credentials silently.
    ...
    /// Which server's documents these are. One value today; the column exists so that many-servers-sharing is a migration rather than a rewrite.
    pub scope: String,

    /// Largest document body accepted, in bytes.
    pub max_body_bytes: usize,

    /// Requests per minute per route before the bridge starts refusing. The caller is a game host, and a compromised host is the thing being limited.
...
impl Default for BridgeConfig {
    fn default() -> Self {
        Self {
            ...
            // The five documents are small; a character profile is a few KB. A megabyte is generous and still bounds a hostile write.
...
pub enum AuthMode {
    ...
    /// The trust boundary is the process tree, not the token: the gamemode's own design guarantees only the game host ever talks to the bridge, and Cellar is the process that launched that host on a loopback bind. This is honest about what it does rather than claiming a verification that has no published endpoint to perform.
    Trusted,
    /// Require a configured shared secret. For a bridge reachable off-box. Needs a matching gamemode-side change to send it.
    SharedSecret,
    /// Verify the token with Facepunch. Unimplemented: no public introspection endpoint for `Auth.GetToken` was found. Selecting it is refused at load rather than silently downgraded.
    Facepunch,
}
...
pub struct DatabaseConfig {
    ...
    /// Optional path to a one-line external MySQL or MariaDB URL. The file is read only when `CELLAR_DATABASE_URL` is absent and is never serialized.
    pub url_file: Option<PathBuf>,
    pub max_connections: u32,
    /// Who owns the tables in this connection. Game-owned is the safe default: Cellar can inspect and query the schema, but never invents game tables.
    pub schema_owner: DatabaseSchemaOwner,
    /// Legacy opt-in for Cellar's operational migrations. It is ignored for a game-owned database and remains only for compatible v0.1 configs.
    pub migrate_on_start: bool,
    /// Permit authenticated operators to apply one validated data or schema statement through the database panel.
...
/// A MariaDB server Cellar downloads, initializes, and supervises itself, rather than only connecting to one hosted elsewhere.
/// Deliberately independent of `[database]`: this section only decides whether `cellar run` also spawns and supervises a `mariadbd` process. `database.url` (`CELLAR_DATABASE_URL`) is still the one source of the connection string everything else uses, whether it points at a managed instance or a remote one. Managed provisioning writes the generated URL to `database.url_file` when that path is configured.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields, default)]
pub struct MariaDbConfig {
    /// Download (if needed), initialize (if needed), and supervise a local instance for the lifetime of `cellar run`.
    pub managed: bool,

    /// Exact version to install, e.g. "11.4.5". Never resolved from "latest": a server manager that silently tracks a moving target is a supply-chain surprise, the same reasoning `update.policy` defaults away from `Apply` for the game itself.
    pub version: String,

    /// sha256 of the official win64 archive for `version`, checked once by whoever sets `version` against MariaDB's own published hashes. The download is verified only against this pinned value, never against a checksum fetched over the same connection as the archive.
    pub sha256: Option<String>,

    /// Where the versioned binaries are unpacked. Required when `managed`.
    pub install_dir: Option<PathBuf>,

    /// Where the data directory lives, independent of `install_dir` so a version upgrade never touches it. Required when `managed`.
    pub data_dir: Option<PathBuf>,

    /// Loopback port. Not 3306: a machine that already has something bound there, a stray prior MariaDB or MySQL install, should not collide with a managed instance silently choosing the same port.
    pub port: u16,

    /// Database and user created during provisioning. Restricted to `[A-Za-z_][A-Za-z0-9_]*` by `validate()`, since both are interpolated into the bootstrap `CREATE DATABASE`/`CREATE USER` statements.
    pub database: String,
    pub username: String,
    ...
    /// How long a graceful stop waits for `mariadb-admin shutdown` to finish before killing the process outright.
    pub graceful_timeout_seconds: u64,
}
...
pub struct BackupConfig {
    ...
    /// On by default, and it costs a few milliseconds. The failure it catches is a disk that filled up partway through writing, which leaves a plausible file of a plausible size that restores into a half-applied database.
    pub verify: bool,

    /// A second copy, somewhere that is not this disk.
    /// A directory, because whatever is mounted there is the operator's decision: a network share, another volume, a synced folder. Cellar has no business holding credentials for an object store whose contents it cannot check.
    pub copy_to: Option<PathBuf>,

    /// Take a dump before applying a game update.
    /// The one moment a rollback is most likely to be wanted and least likely to have been planned for.
    pub before_update: bool,
}

/// Optional snapshots of the gamemode documents Cellar stores through the generic bridge. The format contains no gamemode schema, so any bridge client can export and restore its own documents without Cellar knowing what they mean.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields, default)]
...
/// Cellar never invents an s&box editor command. The editor owns the Steam session, so operators provide the exact commands their project supports.
...
pub enum ConfigError {
    ...
    Parse {
        path: PathBuf,
        // Boxed: `toml::de::Error` alone pushes this variant past clippy's `result_large_err` threshold, which would otherwise flag every `Result<_, ConfigError>` in this module, `load` and `validate` included.
...
impl Config {
    ...
    /// Parse config text as if it had been read from `path`, and resolve `profile_file` against it.
    /// Split out from [`Config::load`] because a plain `toml::from_str` leaves `profile_file` unresolved, which reads as "this config has no profile" and is exactly the mistake a test over the shipped profiles exists to catch. Anything that parses a config without going through here gets a half-loaded one.
    ...
    /// Relative to the config file's own directory, not the process working directory: a config is started from a service unit, a shell and a container with three different working directories, and a path that means something different in each is a path that works until it does not.
    ...
    /// Which server's data this is, for the bridge's `scope` column and the operations tables. The primary instance's scope.
    ...
    /// The load-bearing property is in the legacy arm: a `[server]` config takes its scope from the existing global `bridge.scope`, **not** from its id. Every shipped profile therefore keeps writing to exactly the scope it writes to today, so gaining this feature moves no rows and runs no migration.
    ...
    /// The first enabled one in id order, so a config that disables its development instance on Linux still has a primary.
    ...
    /// The primary's server settings, for a caller that has not been taught about instances yet.
    ...
    /// Secrets are only ever read here. Nothing else in Cellar reads an environment variable, so "where does this credential come from" has one answer.
    pub fn overlay_env(&mut self) {
        // Every instance shares one GSLT today. Steam issues a token per app account rather than per server process, and the engine takes it as a launch argument, so a per-instance override would be a setting with nothing behind it.
    ...
    /// Refuse a configuration that would fail later, in a way that is harder to diagnose than a message at startup.
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        self.validate_exclusive_resources()?;

        Ok(())
    }

    /// Re-check filesystem-backed instance isolation before startup mutates it.
    pub fn validate_exclusive_resources(&self) -> Result<(), ConfigError> {
        refuse_shared_resources(&self.exclusive_resources())
    }

    /// What each running instance must hold to itself.
    /// Disabled instances are excluded: one that is declared but not started holds nothing, and that is exactly how a Windows-only development instance stays in a config file that also deploys to Linux.
...
/// Sharing any of these is not a degraded configuration, it is two servers writing over each other, and every one of them has a silent failure mode: two tailers parsing both servers' lines, one `hosting.json`, one document scope taking both servers' writes, one socket.
#[derive(Debug)]
struct Exclusive {
    id: String,
    log_file: PathBuf,
    ...
    /// `Some` only under `direct_connect`. Without it the engine reaches players through Steam's relay and the port is not a shared resource.
    direct_port: Option<u16>,
    /// `Some` only under the Wine launcher, where the prefix is a real shared resource rather than an unused setting.
    wine_prefix: Option<PathBuf>,
}

fn refuse_shared_resources(instances: &[Exclusive]) -> Result<(), ConfigError> {
    // The invariant that keeps this safe to add before it is reachable: a config with one instance cannot collide with itself, and every config that exists today has one. Nothing below may fire for them.
    if instances.len() < 2 {
        return Ok(());
    }
    ...
    for (index, one) in instances.iter().enumerate() {
        for other in &instances[index + 1..] {
            ...
            if managed_paths_equal(&one.log_file, &other.log_file)? {
                return Err(clash(
                    "the log file",
            ...
            if let (Some(one_dir), Some(other_dir)) = (&one.data_dir, &other.data_dir)
                && managed_paths_equal(one_dir, other_dir)?
            {
                return Err(clash(
                    "the data directory",
            ...
            if let (Some(one_prefix), Some(other_prefix)) = (&one.wine_prefix, &other.wine_prefix)
                && managed_paths_equal(one_prefix, other_prefix)?
            {
                return Err(clash(
                    "the wine prefix",
...
fn managed_paths_equal(one: &Path, other: &Path) -> Result<bool, ConfigError> {
    let one = managed_path_identity(one)?;
    let other = managed_path_identity(other)?;
    #[cfg(windows)]
    return Ok(one
        .to_string_lossy()
        .replace('/', "\\")
        .eq_ignore_ascii_case(&other.to_string_lossy().replace('/', "\\")));
    #[cfg(not(windows))]
    Ok(one == other)
}

fn managed_path_identity(path: &Path) -> Result<PathBuf, ConfigError> {
    let absolute = if path.is_absolute() {
        ...
        std::env::current_dir()
            .map_err(|error| {
                ConfigError::Invalid(format!(
                    "could not resolve managed path {} from the working directory: {error}",
                    path.display()
                ))
            })?
            .join(path)
    ...
    let mut identity = std::fs::canonicalize(&ancestor).map_err(|error| {
        ConfigError::Invalid(format!(
            "could not resolve managed path {}: {error}",
            path.display()
        ))
    })?;
...
/// Every message names the table it is talking about, because with several instances "server.executable is required" would not say whose.
...
/// `mariadb.database`/`mariadb.username` can't be bind parameters: MariaDB has no placeholder syntax for identifiers in `CREATE DATABASE`/`CREATE USER`. Restricting the charset before the name ever reaches a query is what makes that interpolation safe, the same reasoning `admin.rs`'s table browser uses for validating a table name against `tables()` before interpolating it.
...
mod tests {
    ...
    fn a_local_project_profile_still_finds_the_directory_the_operator_configured() {
        // The whole of the bug: the derived path needs server.game, a local .sbproj run does not set it, and the Access panel then read features.json and permissions.json as their empty fallbacks while the operator had already named the directory.
    ...
    /// Every AppleJack Framework profile shipped with the `#local` leaf, published ones included, until 2026-08-28, and the published Facepunch Sandbox profiles slipped past the first version of this test because it only read `applejackrp*` files. Reading the files is the only way to catch any of them: nothing else in the workspace loads them.
    #[test]
    ...
    /// Six AppleJack Framework configs point at one profile file, so this asserts the link resolves rather than silently reading as "no profile".
    #[test]
    ...
    /// A `server.ready_pattern` is the narrower statement and has to win, or an operator who overrode it for one instance would find the profile quietly putting it back.
    #[test]
    ...
    /// An empty string is a real answer, not an absent one: it is how an operator says this gamemode has no readiness signal. A defaulted `String` could not tell the two apart, which is why the field is an `Option`.
    #[test]
    ...
    /// Every profile that ships, parsed. Reading the files is the only way to catch a mistake in one: nothing else in the workspace loads them.
    fn shipped_profiles() -> Vec<(String, Config)> {
        ...
        // `install.sh`, `install.ps1` and the container entrypoint all copy this one into place as an operator's starting config, and until 2026-09-01 nothing in the workspace parsed it.
        sources.push((
        ...
        // The Kubernetes manifest embeds the same profile as a ConfigMap; pull the indented block back out rather than leaving it unchecked.
    ...
    /// The invariant the whole refusal rests on. Every config that can be written today has one instance, so none of the refusals can reach a deployment that starts now. Remove this and the collision checks become a way to break a working server.
    #[test]
    ...
    fn every_shipped_profile_still_resolves_to_exactly_one_instance() {
        for (name, config) in shipped_profiles() {
            ...
            // The load-bearing property of the desugaring. A legacy config takes its scope from the global bridge.scope, never from its id, so gaining the instance model moves no rows and runs no migration. If this ever fails, a deployment's documents are about to move.
            assert_eq!(
    ...
    fn an_instance_may_name_a_scope_that_is_not_its_id() {
        // The id is the routing key and the scope is the storage key. An existing deployment names its old scope here so its documents stay where they are while it gains a second instance.
        let config = parse(
            r#"
            [instances.published]
...
",
    ...
    fn a_typo_inside_an_instance_is_refused_rather_than_ignored() {
        // The whole reason the server table is nested rather than flattened: serde cannot apply deny_unknown_fields through a flatten, and a silently ignored `hostnam` in this table is a server with the wrong name and no way to find out why.
        let error = parse(
            r#"
            [instances.dev.server]
            executable = "/srv/sbox-server.exe"
    ...
    fn the_collision_refusals_are_reachable_once_a_config_has_two_instances() {
        // Phase 1 wrote these before anything could reach them. This is the moment they start mattering.
        let config = parse(
            r#"
            [instances.dev.server]
    ...
    fn a_native_instance_has_no_prefix_to_collide_over() {
        // The field exists on every server config, and on Windows it means nothing. Comparing it there would refuse a working two-instance Windows config for a setting neither instance uses.
        let config = parse(
            r#"
            [instances.dev.server]
    ...
    fn a_disabled_instance_holds_nothing_and_so_cannot_collide() {
        // How a Windows-only development instance stays in a config file that also deploys to Linux.
        let config = parse(
            r#"
            [instances.dev]
            enabled = false
    ...
    #[test]
    fn managed_paths_match_through_relative_and_parent_components() {
        let current = std::env::current_dir().unwrap();
        let root = tempfile::Builder::new()
            .prefix("cellar-path-identity-")
            .tempdir_in(&current)
            .unwrap();
        ...
        let relative = root
            ...
            .unwrap()
            .join("real/data");
        let parented = root.path().join("real/../real/data");

        assert!(managed_paths_equal(&relative, &parented).unwrap());
        assert!(!managed_paths_equal(&relative, &root.path().join("other/data")).unwrap());
    }

    #[cfg(unix)]
    #[test]
    fn managed_paths_match_through_a_symlinked_existing_parent() {
        ...
        assert!(
            managed_paths_equal(&real.join("future/log.txt"), &alias.join("future/log.txt"))
                .unwrap()
        );
    }
    ...
    #[cfg(windows)]
    #[test]
    fn managed_paths_match_through_a_junction() {
        let root = tempfile::tempdir().unwrap();
        let real = root.path().join("real");
        let alias = root.path().join("alias");
        std::fs::create_dir(&real).unwrap();
