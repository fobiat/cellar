//! First-run and re-run setup: install the binaries, initialize the data directory, and create the `cellar` database, user and password.
//! Safe to call more than once. Installing and initializing are skipped when already done (see `release::already_installed` and the data-dir check below), but the database/user/grant step always runs and always resets the password, so `cellar mariadb provision` is also how an operator who lost `CELLAR_DATABASE_URL` recovers it: root has no password (a deliberate property of `--auth-root-authentication-method=normal`, safe because this server never binds anything but loopback), so re-running this can always reach it and issue a fresh password.
...
/// Non-secret state written after a successful provision, so `status` can report what is there without touching the data directory.
...
/// Absent or unreadable is not an error here: an empty or missing data directory is exactly the "not provisioned yet" case `provision` already handles, and a marker that fails to parse is treated the same way rather than blocking a re-provision that would fix it.
...
/// `database`/`username` are trusted here: `Config::validate` refuses anything that does not match `[A-Za-z_][A-Za-z0-9_]*` before a config carrying `mariadb.managed = true` is ever loaded, which is what makes interpolating them directly safe. `password` comes only from `credentials::generate_password`, alphanumeric by construction, so it needs no escaping either.
fn bootstrap_sql(database: &str, username: &str, password: &str) -> Vec<String> {
    vec![
        ...
        format!("REVOKE ALL PRIVILEGES, GRANT OPTION FROM '{username}'@'127.0.0.1'"),
        format!(
            "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX, REFERENCES, \
             LOCK TABLES, CREATE VIEW, SHOW VIEW, TRIGGER, EVENT, CREATE ROUTINE, \
             ALTER ROUTINE, EXECUTE ON `{database}`.* \
             TO '{username}'@'127.0.0.1'"
        ),
        format!("GRANT SELECT ON `mysql`.`proc` TO '{username}'@'127.0.0.1'"),
        // So `supervisor.rs` can stop mariadbd cleanly as this same user, rather than keeping a root credential around after provisioning.
        format!("GRANT SHUTDOWN ON *.* TO '{username}'@'127.0.0.1'"),
        "FLUSH PRIVILEGES".to_owned(),
    ]
}

/// Install (if needed), initialize (if needed), and (re-)create the database, user and password. Returns the `mysql://` URL for `CELLAR_DATABASE_URL`.
pub async fn provision(
    config: &MariaDbConfig,
    client: &reqwest::Client,
    existing_password: Option<&str>,
) -> Result<String, ProvisionError> {
    // Before the download, not after: `release::archive_url` serves the winx64 zip unconditionally, and every binary this module spawns is named `.exe`. Without this the caller pays a 400MB download, unpacks Windows binaries onto a Linux host, and only then fails on the first exec with a bare "Permission denied (os error 13)" that names nothing.
    if !cfg!(windows) {
    ...
    // The system schema's own directory is the simplest reliable sign that `mariadb-install-db` has already run here.
    ...
    // Stop the bootstrap server whether or not the SQL succeeded, so a failed provision never leaves a stray mariadbd holding the port.
...
/// Windows' `mariadb-install-db.exe` is not the Unix `mariadb-install-db` shell script: it is a distinct binary (internally still `mysql_install_db Ver 1.00`, from before the MariaDB/MySQL rename) with a much smaller option set, no `--auth-root-authentication-method`, no `--skip-test-db`, confirmed against the real distribution rather than assumed. `--datadir` is the only thing it needs; the result leaves `root` with an empty password over TCP unless `-p`/`-R`/`-N` say otherwise, which is exactly the bootstrap access `provision` relies on.
...
/// Spawn `mariadbd`, loopback-bound. Shared with `supervisor.rs`, which spawns the same way for the long-running instance `cellar run` supervises.
pub(crate) fn spawn(
    bin_dir: &Path,
    data_dir: &Path,
    port: u16,
...
async fn run_root_sql(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Dropping `stdin` here closes the pipe, which is how the client knows there is no more input coming.
    }
...
/// `mariadb-admin shutdown`, as either root (bootstrap) or the app user (`supervisor.rs`'s graceful stop). Errors are deliberately swallowed by both callers: a shutdown that fails leaves the process to be killed outright, which is degraded but not unsafe.
...
mod tests {
    ...
    fn bootstrap_sql_creates_the_database_user_and_grants() {
        ...
        assert!(joined.contains("REVOKE ALL PRIVILEGES, GRANT OPTION FROM 'cellar'@'127.0.0.1'"));
        assert!(joined.contains(
            "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, DROP, INDEX, REFERENCES, \
             LOCK TABLES, CREATE VIEW, SHOW VIEW, TRIGGER, EVENT, CREATE ROUTINE, \
             ALTER ROUTINE, EXECUTE ON `cellar`.* \
             TO 'cellar'@'127.0.0.1'"
        ));
        assert!(joined.contains("GRANT SELECT ON `mysql`.`proc` TO 'cellar'@'127.0.0.1'"));
        assert!(!joined.contains("GRANT ALL PRIVILEGES"));
    ...
    fn bootstrap_sql_is_idempotent_by_construction() {
        // `IF NOT EXISTS` / `ALTER` rather than a bare `CREATE USER`, so a re-run against an already-provisioned instance does not fail on the second statement.
