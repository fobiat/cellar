...
//! The gamemode authenticates with `Sandbox.Services.Auth.GetToken(audience)`, presented as `Authorization: Bearer …`. That is a good scheme: the token is platform-issued at runtime, so nothing secret ships inside the game package, which matters because a game package is distributed to every client and any secret compiled into one is public by construction.
//! The catch is the other end. Facepunch's service API publishes package, version, stats, leaderboard, achievement, player, news, notification, storage, utility and code endpoints, and **no token introspection**. There is no documented way for a third party to verify one of these tokens.
//! So Cellar does not claim to. [`AuthMode::Trusted`] requires a well-formed bearer and records it, and leans on the fact that the bridge binds loopback and Cellar is the process that launched the only client allowed to reach it. The trust boundary is the process tree, not the token, and the config refuses to start `Trusted` on a reachable address so that boundary stays real.
...
impl Policy {
    ...
    /// `AuthMode::Facepunch` cannot be built: the config layer refuses it at load with an explanation, and this returns an error rather than silently downgrading to something weaker. A security mode that quietly becomes a different mode is worse than one that refuses to start.
...
/// Not a security check. It catches an empty or truncated header, which is the realistic misconfiguration, and says so rather than pretending to more.
...
pub(crate) fn constant_time_eq(a: &[u8], b: &[u8]) -> bool {
    ...
    for (index, expected) in b.iter().enumerate() {
        difference |= usize::from(a.get(index).copied().unwrap_or_default() ^ expected);
    }
    difference == 0
}
