...
//! The better of the two channels to parse. The console truncates every logger name to eight characters and carries a 12-hour clock with no date; the file is tab separated, dated to four decimal places, and keeps the logger name whole.
...
//! - `ArchiveOldFileOnStartup = true`, so every restart renames the file away and begins a new one;
...
pub struct Tailer {
    ...
    position: u64,
    partial: Vec<u8>,
    /// How many times the file was seen to rotate. Surfaced so an operator can tell "the log went quiet" from "the log moved".
    rotations: u64,
    /// Whether the follow position has been fixed yet.
    /// Only the first poll may skip to the end, and only if the file already existed. A file that appears later belongs entirely to this run, so it is read from its first byte: seeking to the end of a file the server has just created loses every line it wrote while starting up, which is most of the lines worth having.
    armed: bool,
}

impl Tailer {
    /// Follow `path`, starting at its end.
    /// From the end, not the beginning: a restart must not replay a 500 MB archive as if every historical player had just joined.
    ...
    /// Never blocks and never fails: a missing or unreadable file is "nothing new yet", because the common case for both is a server that has not written its first line.
    pub fn poll(&mut self) -> Vec<String> {
        if self.reader.is_none() {
            // Skip to the end only on the very first poll, and only if the file was already there. A file created after arming is this run's own.
        ...
        // A file shorter than where we are reading was rotated or truncated underneath us. Start the new one from its beginning.
    ...
    /// The engine's last line before a crash is often unterminated, and it is usually the one worth reading.
