...
pub async fn doctor(path: &Path) -> Result<()> {
    ...
    for check in &report.checks {
        let label = match (&check.instance, several) {
            (Some(instance), true) => format!("{instance}: {}", check.label),
            _ => check.label.clone(),
        };
        let word = match check.outcome {
            cellar_diagnostics::Outcome::Ok => "ok  ",
            cellar_diagnostics::Outcome::Fail => "FAIL",
            cellar_diagnostics::Outcome::Note => "note",
        };
        println!("  {word}  {label}: {}", check.detail);
    }
...
async fn refuse_if_a_cellar_is_running(config: &Config) -> Result<()> {
    ...
    if !matches!(
        cellar_diagnostics::address_is_free(&config.web.bind).await,
        cellar_diagnostics::Free::HeldByCellar
    ) {
        return Ok(());
    }
